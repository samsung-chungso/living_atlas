...
WATER_TEMP_FEATURE_ID = "c7ce139631bb46a3b264f7d0e43bd80c"
...
def build_update_data(gis: GIS) -> list[dict]:
    layer_data = get_layer_data(gis, WATER_TEMP_FEATURE_ID)
    ...
    for feature in tqdm(layer_data):
        ...
        try:
            ...
            feature.attributes["water_temp"] = parse(latest_record["water_temp"])
        except KeyError as e:
            print(e)
            continue
    return layer_data
...
def update(gis: GIS, layer_data: list[dict]) -> None:
    layer = get_flc_by_id(gis, WATER_TEMP_FEATURE_ID).layers[0]
